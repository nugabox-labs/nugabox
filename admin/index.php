    ...
    <div class="card-soft">
      ...
      <div class="cb">
        <div class="stat-list">
          ...
          <div class="row-i"><span class="k">쓰기 권한</span><span class="v"><?= is_writable($uploadDir) ? '있음' : '없음' ?></span></div>
          <div class="row-i"><span class="k">설정 파일</span><span class="v"><?= h(env_file() ?? '없음 — 기본값으로 동작 중') ?></span></div>
          <div class="row-i"><span class="k">PHP</span><span class="v"><?= h(PHP_VERSION) ?></span></div>
        </div>
        <?php if ($git['clean'] === false): ?>
          <p class="hint-text">
